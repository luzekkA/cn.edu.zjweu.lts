<template>
  ...
  <el-row>
    ...
    <el-col :span="5" v-for="(item, index) in CoursesList.filter(item => item.Name.includes(keyword))"
      ...
      <router-link :to="{ path: '/Student/CourseTopic', query: { CourseId: item.Id } }">
        <div class="myClass">
          ...
          <el-image style=" width:280px; height:150px; border-radius: 5px;" :src="'/ClassBackGround.png'" :fit="fit" v-bind:style="{ backgroundColor: getColorById(item.Id) }" />
        </div>
      </router-link>
    </el-col>
  </el-row>
...
onMounted(() => {
  getCourses(currentPage.value).then(data => {
    devLog("this is data",data.data.data.Items)
